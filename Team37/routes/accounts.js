...
const {DisplayUser, getUserOrders, getDisplayUser} = require("../utilities/dbUtilities");
const { CartItem, Prop} = require("../utilities/models");
...
router.post("/login", isVerified, (req, res, next) => {
    ...
    passport.authenticate("local", { // this method passport.authenticate is used to check a user's entered credentials and log them in
        // this also creates a new session and attaches that sessionId to the cookie
        ...
        if (req.isAuthenticated() && cartData) {
            const cartItemsToAdd = JSON.parse(cartData);
            console.log(cartData)
            cartItemsToAdd.forEach((item) => {
                if (Prop.findById(item)) {
                    req.user.cart.push(new CartItem({
                        itemId: item,
                        quantity: 1
                    }));
                }
...
router.post("/:userId/update-lname", isAuth, async (req, res) => {
    ...
    try {
        if (newLname === "") throw new Error("Updated Last name Cannot be empty");
...
router.post("/:userId/update-email", isAuth, async (req, res) => {
    ...
    try {
        if (newEmail === "") throw new Error("Updated Email Cannot be empty");
...
router.post("/:userId/update-phone", isAuth, async (req, res) => {
    ...
    try {
        if (newPhone === "") throw new Error("Updated Phone Cannot be empty");
